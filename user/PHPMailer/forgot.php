...
require 'src/Exception.php'; // Passing `true` enables exceptions
...
class Mailer
{
    ...
    {
        $addressmail = new PHPMailer\PHPMailer\PHPMailer(true); //true:enables exceptions
        try {
            $addressmail->SMTPDebug = 0;
            $addressmail->isSMTP();
            $addressmail->CharSet = "utf-8";
            $addressmail->Host = 'smtp.gmail.com'; //SMTP servers
            $addressmail->SMTPAuth = true; // Enable authentication
            $addressmail->Username = 'lephuocminh12321@gmail.com'; // SMTP username
            $addressmail->Password = 'pvlqozspjhltmjpn'; // SMTP password
            $addressmail->SMTPSecure = 'ssl';
            $addressmail->Port = 465; // port to connect to
            $addressmail->setFrom('lephuocminh12321@gmail.com', 'E-SHOP');
            $addressmail->addAddress($email);
            $addressmail->isHTML(true);
            $addressmail->Subject = $title;
            $addressmail->Body = $content;
//            $addressmail->smtpConnect(
//                array(
//                    "ssl" => array(
//                        "verify_peer" => false,
...
//            );
            $addressmail->send();
            echo '<span class="text-success">Đã gửi mail xong</span>';
        } catch (Exception $e) {
            echo 'Error: ', $addressmail->ErrorInfo;
        }
    }

    public function thanksMail($username, $email)
    {
        $addressmail = new PHPMailer\PHPMailer\PHPMailer(true); //true:enables exceptions
        try {
            $addressmail->SMTPDebug = 0; //0,1,2: chế độ debug
            $addressmail->isSMTP();
            $addressmail->CharSet = "utf-8";
            $addressmail->Host = 'smtp.gmail.com'; //SMTP servers
            $addressmail->SMTPAuth = true; // Enable authentication
            $addressmail->Username = 'lephuocminh12321@gmail.com'; // SMTP username
            $addressmail->Password = 'pvlqozspjhltmjpn'; // SMTP password
            $addressmail->SMTPSecure = 'ssl'; // encryption TLS/SSL
            $addressmail->Port = 465; // port to connect to
            $addressmail->setFrom('lephuocminh12321@gmail.com', 'E-Shop');
            $addressmail->addAddress($email);
            $addressmail->isHTML(true);
            $addressmail->Subject = 'Thư cảm ơn';
            ...
            box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
                ...
                <p style="margin-bottom: 20px;">Kinh chào ' . $username . ',</p>
                ...
                <p>Chúng tôi sẽ phản hồi với khách hàng trong thời gian sớm nhất, hoặc quý khách có thể liên hệ trực tiếp  qua Email :  <a style="color: #007bff;
                    text-decoration: none;"
                        href="lephuocminh12321@gmail.com">lephuocminh12321@gmail.com</a> và SĐT 0123456789  .</p>
                <p>Trân trọng,<br>E-Shop</p>
            </div>
        ';
            $addressmail->Body = $noidungthu;
            $addressmail->smtpConnect(
                array(
                    "ssl" => array(
                        "verify_peer" => false,
    ...
    public function ReplyMail($username, $email, $subject, $question, $answer)
    {
        $addressmail = new PHPMailer\PHPMailer\PHPMailer(true); //true:enables exceptions
        try {
            $addressmail->SMTPDebug = 0; //0,1,2: chế độ debug
            ...
            $addressmail->SMTPSecure = 'ssl'; // encryption TLS/SSL
            $addressmail->Port = 465; // port to connect to
            $addressmail->setFrom('lephuocminh12321@gmail.com', 'Liberty Technology');
            $addressmail->addAddress($email);
            $addressmail->isHTML(true);
            $addressmail->Subject = 'Thư phản hồi khách hàng';
            $noidungthu = ' 
    ...
    public function MailOrder($username, $email)
    {
        $addressmail = new PHPMailer\PHPMailer\PHPMailer(true); //true:enables exceptions
        try {
            $addressmail->SMTPDebug = 0; //0,1,2: chế độ debug
            ...
            $addressmail->addAddress($email);
            $addressmail->isHTML(true);
            $addressmail->Subject = 'Thông tin đơn hàng';
